...
// TODO: display errors reported via flash_error right in the text editor window
// somehow
#define flash_error(...)                                                       \
  do {                                                                         \
...
int main(int argc, char **argv) {
  ...
  SDL_Window *window =
      SDL_CreateWindow("ded", 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
  ...
  while (!quit) {
    ...
    while (SDL_PollEvent(&event)) {
      ...
      case SDL_KEYDOWN: {
        if (file_browser) {
          ...
          case SDLK_RETURN: {
            ...
            if (file_path) {
              ...
              if (err != 0) {
                ...
                case FT_REGULAR: {
                  // TODO: before opening a new file make sure you don't have
                  // unsaved changes And if you do, annoy the user about it.
                  // (just like all the other editors do)
                  err = editor_load_from_file(&editor, file_path);
                  if (err != 0) {
          ...
          case SDLK_TAB: {
            // TODO: indent on Tab instead of just inserting 4 spaces at the
            // cursor That is insert the spaces at the beginning of the line.
            // Shift+TAB should do unindent, that is remove 4 spaces from the
      ...
      case SDL_TEXTINPUT: {
        if (file_browser) {
          // Nothing for now
          // Once we have incremental search in the file browser this may become
          // useful
        } else {
...

    {
      ...
      // TODO(#19): update the viewport and the resolution only on actual window
      // change
      glViewport(0, 0, w, h);
    }
